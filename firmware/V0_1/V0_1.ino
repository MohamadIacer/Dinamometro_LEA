/* ----------------------------------------------------------------------------
 *  Projeto:   Dinamômetro Aerogerador – Módulo de Controle e Aquisição
 *  Firmware:  Controle FOC + Aquisição ADS1256 + Encoder AS5047 + ADXL345
 *  Placa:     ESP32 DevKit V1 
 *  Versão:    v0.1
 *  Autor:     Mohamad Iacer Taleb Fares (LEA – UnB)
 *  Data:      2025.2
 *
 *  Descrição:
 *    Firmware responsável por:
 *      - Controle vetorial de campo (FOC) do atuador BLDC (SimpleFOC)
 *      - Leitura de posição via AS5047 (SPI)
 *      - Leitura dos extensômetros pelo ADS1256 (24 bits)
 *      - Leitura do acelerômetro ADXL345 (SPI)
 *      - Aquisição em múltiplos modos e envio de dados via serial
 *      - Execução distribuída em dois núcleos (Core 0 e Core 1)
 *
 *  Núcleos:
 *      Core 1 → Controle em tempo real (FOC)
 *      Core 0 → Aquisição, comunicação e processamento
 *
 *  Observações:
 *      - Todos os sensores operam no barramento VSPI.
 *      - Limite seguro de tensão do driver: 4.5 V.
 *      - Delays mínimos foram adicionados para evitar disputa no barramento SPI.
 *
 *  --------------------------------------------------------------------------- */
